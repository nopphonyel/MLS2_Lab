...
def predict_transform(prediction, inp_dim, anchors, num_classes, use_cuda=True):
...
def write_results(prediction, confidence, num_classes, nms=True, nms_conf=0.4):
    ...
    try:
        ...
        print('Failed in torch.nonzero: ' + str(e))
        return 0
    ...
    for ind in range(batch_size):
        ...
        try:
            ...
            print('Error running unique(): ' + str(e))
            continue

        # Class-wise non-maxima suppression

        for cls in img_classes:
            # Get the detections for this class
            ...
            if nms:
                for i in range(idx):
                    # Get the IOUs of this box with all the others that come later
                    try:
                        ...
                        break
                    # Zero out all the detections with IoU > treshhold
                    iou_mask = (ious < nms_conf).float().unsqueeze(1)
                    image_pred_class[i + 1:] *= iou_mask
                    # Remove the non-zero entries
                    non_zero_ind = torch.nonzero(image_pred_class[:, 4]).squeeze()
                    image_pred_class = image_pred_class[non_zero_ind].view(-1, 7)
            # Make a list of all detections for this batch. Add the batch_id of each
            # image so we can sort it out later.
...
def bbox_iou(box1, box2):
    ...
    if torch.cuda.is_available():
        ...
        inter_area = torch.max(inter_rect_x2 - inter_rect_x1 + 1, torch.zeros(inter_rect_x2.shape)) * \
                     torch.max(inter_rect_y2 - inter_rect_y1 + 1, torch.zeros(inter_rect_x2.shape))
    # Get union area
...
def predict_transform_half(prediction, inp_dim, anchors, num_classes, use_cuda=True):
    batch_size = prediction.size(0)
    stride = inp_dim // prediction.size(2)
    bbox_attrs = 5 + num_classes
    num_anchors = len(anchors)
